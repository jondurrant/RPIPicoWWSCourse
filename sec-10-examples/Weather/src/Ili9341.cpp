...
void Ili9341::test(){
	...
	for (int i=0; i < PIX_WIDTH; i++){
	 ...
	 for (int i=0; i < PIX_HEIGHT; i++){
		 if (i < 100){
			 for (int j=0; j < PIX_WIDTH/3; j++){
				 sBufLine[j] = RGBto16bit(0xff, 0, 0);
			 }
		 } else  if (i < 200){
			 for (int j=0; j < PIX_WIDTH/3 * 2; j++){
				 sBufLine[j] = RGBto16bit(0, 0xff,0);
			 }
		 } else {
			 for (int j=0; j < PIX_WIDTH; j++){
				 sBufLine[j] = RGBto16bit(0, 0, 0xff);
			 }
		 }
...
void Ili9341::clear(){
	drawRect(0, 0, PIX_WIDTH, PIX_HEIGHT, 0);
}
