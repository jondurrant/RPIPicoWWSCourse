...
int Digits::drawSymbol(char  sym, uint8_t x, uint8_t y,  PicoGraphics *graphics){
	...
	if ((sym >= '0') && (sym <= '9')){
		...
		case 'c':
			...
			break;
		case '$':
			res = drawSprite(DIGITS_DOLLAR,  x,  y,   graphics);
			break;
		case 'E':
			res = drawSprite(DIGITS_EURO,  x,  y,   graphics);
			break;
		case 'P':
			res = drawSprite(DIGITS_POUND,  x,  y,   graphics);
			break;
		}
	}
	return res;
...
int Digits::drawDollars(float f ,  uint8_t x, uint8_t y, PicoGraphics *graphics){
	int col = 0;

	col+=drawSymbol('$', x+col, y, graphics);
	col++;
	col+=drawFloat(f, x+col, y, graphics,  false, false, 1, 2);

	return col;

}

int Digits::drawEuros(float f , uint8_t x, uint8_t y,  PicoGraphics *graphics){
	int col = 0;

	col+=drawSymbol('E', x+col, y, graphics);
	col++;
	col+=drawFloat(f, x+col, y, graphics, false, false, 1, 2);
...
int Digits::drawPounds(float f ,  uint8_t x, uint8_t y, PicoGraphics *graphics){
	int col = 0;

	col+=drawSymbol('P', x+col, y, graphics);
	col++;
	col+=drawFloat(f, x+col, y, graphics, false, false, 1, 2);
